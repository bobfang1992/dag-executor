...
export function generateCapabilitiesTs(capabilities: CapabilityEntry[], digest: string): string {
  ...
  lines.push("");
  lines.push("  // null schema means no payload allowed");
  lines.push("  if (schema === null) {");
  lines.push('    if (payload !== undefined && payload !== null && (typeof payload !== "object" || Object.keys(payload as object).length > 0)) {');
  lines.push("      return `Capability ${capId} does not allow a payload`;");
  lines.push("    }");
  lines.push("    return null;");
  lines.push("  }");
  lines.push("");
  lines.push("  // Validate object type");
  lines.push('  if (schema.type === "object") {');
  lines.push('    if (typeof payload !== "object" || payload === null) {');
