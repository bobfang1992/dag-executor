...
function paramTypeToTsType(param: TaskParamEntry): string {
  ...
  switch (type) {
    ...
    case "node_ref":
      ...
      break;
    case "endpoint_ref":
      if (param.endpoint_kind === "redis") {
        baseType = "RedisEndpointId";
      } else if (param.endpoint_kind === "http") {
        baseType = "HttpEndpointId";
      } else {
        baseType = "EndpointId";
      }
      break;
...
export function generateTasksTs(registry: TaskRegistry): string {
  // Track endpoint_ref usage and narrowings
  const endpointKindsUsed = new Set<string>();
  let hasGenericEndpointRef = false;
  for (const task of registry.tasks) {
    for (const p of task.params) {
      if (p.type !== "endpoint_ref") continue;
      if (p.endpoint_kind) {
        endpointKindsUsed.add(p.endpoint_kind);
      } else {
        hasGenericEndpointRef = true;
      }
    }
  }
  ...
  if (hasGenericEndpointRef) endpointImports.push("EndpointId");
  if (endpointKindsUsed.has("redis")) endpointImports.push("RedisEndpointId");
  if (endpointKindsUsed.has("http")) endpointImports.push("HttpEndpointId");

  if (endpointImports.length > 0) {
    lines.push(`import type { ${endpointImports.join(", ")} } from "./endpoints.js";`);
  }
...
export function generateTaskImplTs(registry: TaskRegistry): string {
  // Track endpoint_ref usage and narrowings (same as generateTasksTs)
  const endpointKindsUsed = new Set<string>();
  let hasGenericEndpointRef = false;
  for (const task of registry.tasks) {
    for (const p of task.params) {
  ...
  // Add endpoint imports based on usage
  const endpointImports: string[] = [];
  if (hasGenericEndpointRef) endpointImports.push("EndpointId");
  if (endpointKindsUsed.has("redis")) endpointImports.push("RedisEndpointId");
  if (endpointKindsUsed.has("http")) endpointImports.push("HttpEndpointId");
...
export function generateEndpointsTs(endpoints: EndpointEntry[], registryDigest: string): string {
  ...
  lines.push(" * Example: EP.redis.redis_default");
  lines.push(" */");
  ...
  for (const kind of sortedKinds) {
    ...
    for (const ep of eps) {
      ...
      if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(safeName)) {
        throw new Error(
          `Endpoint name "${ep.name}" (id=${ep.endpoint_id}) is not a valid identifier after sanitization (${safeName}).`
        );
      }
      if (RESERVED_IDENTIFIERS.has(safeName)) {
        throw new Error(
          `Endpoint name "${ep.name}" (id=${ep.endpoint_id}) collides with a reserved identifier: ${safeName}`
        );
      }
      if (usedNames.has(safeName)) {
        throw new Error(
          `Endpoint names collide after sanitization within kind "${kind}": ${ep.name} -> ${safeName}`
        );
      }
      usedNames.add(safeName);
      lines.push(`    /** ${ep.name} (${ep.endpoint_id}) */`);
      lines.push(`    ${safeName}: "${ep.endpoint_id}" as EndpointLiteral<"${ep.endpoint_id}">,`);
    }
    lines.push(`  },`);
  }
