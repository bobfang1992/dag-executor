...
function main() {
  ...
  const outputs: Array<{ path: string; content: string }> = [
    ...
    // C++ headers
    { path: "engine/include/keys.h", content: generateKeysH(keys, keysDigest) },
    { path: "engine/include/params.h", content: generateParamsH(params, paramsDigest) },
    { path: "engine/include/features.h", content: generateFeaturesH(features, featuresDigest) },
  ];
