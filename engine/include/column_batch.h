...
class ColumnBatch {
public:
    explicit ColumnBatch(size_t num_rows, std::shared_ptr<DebugCounters> debug = nullptr)
        : id_values_(num_rows), id_valid_(num_rows, 1), debug_(debug ? debug : std::make_shared<DebugCounters>()) {}
