...
Plan parse_plan(const std::string& path) {
    ...
    try {
        ...
        throw std::runtime_error("Invalid JSON in plan file: " + std::string(e.what()));
    }
    ...
    for (const auto& nj : j["nodes"]) {
        ...
        if (!nj.contains("op") || !nj["op"].is_string()) {
            throw std::runtime_error("Node '" + node.node_id + "' missing or invalid 'op'");
        }
        node.op = nj["op"].get<std::string>();

        if (!nj.contains("inputs") || !nj["inputs"].is_array()) {
            throw std::runtime_error("Node '" + node.node_id + "' missing or invalid 'inputs'");
        }
        for (const auto& inp : nj["inputs"]) {
            if (!inp.is_string()) {
                throw std::runtime_error("Node '" + node.node_id + "' has non-string input");
            }
